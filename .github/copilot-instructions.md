...
- MySQL lives in `db` with GTID replication from command→query. Bring databases up via `docker compose up -d command_db query_db db_admin` and seed with `make -C db create-data dump restore start-replication` when tests require fresh data.
...
- Error flow is standardized: domain/application/CRUD/internal errors live in `pkg/errs`; presentation layers translate them to Connect error codes.
