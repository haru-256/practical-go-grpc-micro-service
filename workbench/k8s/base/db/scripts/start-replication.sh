...
mysql -h "$QUERY_DB_HOST" -u root -p"$MYSQL_ROOT_PASSWORD" -e "STOP REPLICA; RESET REPLICA ALL; RESET MASTER;"
...
mysqldump -h "$COMMAND_DB_HOST" -uroot -p"$MYSQL_ROOT_PASSWORD" \
  --single-transaction \
  --set-gtid-purged=ON --routines --triggers \
  --events --hex-blob --all-databases \
  | mysql -h "$QUERY_DB_HOST" -uroot -p"$MYSQL_ROOT_PASSWORD"
...
mysql -h "$QUERY_DB_HOST" -uroot -p"$MYSQL_ROOT_PASSWORD" <<EOF
